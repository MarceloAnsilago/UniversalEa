...
public:
   ...
   void Place(CGuiLayout &layout)
     {
      ...
      m_apply.caption="Aplicar ao motor"; m_toggle.caption=m_active ? "Pausar entradas" : "Ativar ordens";
   ...
   void SetResult(const bool applied,const bool active,const int revision,const string name,const string message,const bool error)
     { m_applied=applied; m_active=active; m_revision=revision; m_applied_name=name; m_message=message; m_error=error;
       m_toggle.enabled=applied; m_toggle.caption=active ? "Pausar entradas" : "Ativar ordens"; }
   ...
   void Render(CGuiRenderer &r,CGuiState &draft,const string symbol)
     {
      ...
      rows[5]="Posições: "+draft.setup.Value(9)+(draft.setup.trade_mode==GUI_SETUP_DAY_TRADE ? " / zerar no fim das entradas" : " / pode manter entre dias");
      ...
      rows[19]="ATIVAR ORDENS HABILITA NEGOCIAÇÃO AUTOMÁTICA.";
      rows[20]="Pausar cancela pendentes; a gestão de posições continua.";
      rows[21]="Day trade zera no fim das entradas; swing mantém posições.";
      for(int i=0;i<22;i++)
        {
      ...
      string state=m_active ? "Ordens ativas" : "Entradas pausadas";
