...
void OnStart()
  {
   ...
   Check(gui.m_rules.state.stop_multiplier==2.1,"Botão mais incrementa em 0,10");
   gui.Click(field.x+field.w-12,field.y+32);
   Check(gui.m_rules.state.stop_multiplier==2,"Botão menos decrementa em 0,10");
   gui.Key(38); gui.Key(38); gui.Key(40);
   Check(gui.m_rules.state.stop_multiplier==2.1,"Setas ajustam o multiplicador");
   gui.m_rules.Begin(19); gui.m_rules.Key(51); gui.m_rules.Key(38);
   Check(gui.m_rules.state.stop_multiplier==3.1,"Incremento confirma valor em edição");
   gui.m_rules.Begin(19); gui.m_rules.Key(189); gui.m_rules.Key(38);
   Check(gui.m_rules.state.stop_multiplier==3.1 && gui.m_rules.m_text[10].invalid,"Texto inválido bloqueia incremento sem perder edição");
